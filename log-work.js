...
const _createDefaultJiraClient = () => {
    const jiraConfigPath = path.resolve(require('os').homedir(), '.jira.config.json')
...
const _getUser = (username) => {
    return jira.user.getUser({ username: 'Chris Chou' })
        .then((user) => {
...
const _getStartTime = (timeSpent) => {
    if (!timeSpent) return
    const now = moment()
    if (timeSpent.match(/^\d+$/)) return _formatMomentJira(now.subtract(+timeSpent, 'minutes'))
